...
export async function POST(request: NextRequest) {
    try {
        ...
        const normalizedEvent = normalizeYCloudWebhookPayload(body, {
            fallbackSourceId,
        });

        if (normalizedEvent.kind === "ignore") {
            return NextResponse.json({ ok: true, ignored: normalizedEvent.reason });
        }
        ...
        if (normalizedEvent.message.direction === "inbound") {
            ...
            return NextResponse.json({ ok: true, stored: "inbound" });
        }
        ...
        return NextResponse.json({ ok: true, stored: "outbound" });
    } catch (error) {
